...
function Login() {
  ...
  const onSubmit = async (data) => {
    ...
    if (json.success) {
      ...
      dispatch(userName(JSON.parse(localStorage.getItem("userInfo")).name));
      setTimeout(() => {
  ...
  return (
    <>
      {loginStatus === true && (
        <Alert colorStatus="success" alertText="Login Successful!!!"></Alert>
      )}
      {loginStatus === false && (
        <Alert
          colorStatus="danger"
          alertText="Invalid email or password. Please try to login with correct credentials."
        ></Alert>
      )}
      <FormHeading />
      <div className="loginForm">
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="loginContainer">
            ...
            <br />
            {errors.email && (
              <div className="loginSpan text-start">
